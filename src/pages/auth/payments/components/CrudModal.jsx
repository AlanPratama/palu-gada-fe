          ...
          {modalType === "Detail" && (
            <Card shadow='none'>
              <CardHeader className='flex-col items-start pb-2'>
                ...
                <p className='text-small text-default-500 mt-1'>Pembayar: {selectedPayment.user.email}</p>
              </CardHeader>
              <Divider className='my-3' />
              <CardBody className='px-0 py-2'>
                <div className='grid grid-cols-1 md:grid-cols-2 gap-4 w-full'>
                  <InfoItem icon='business-outline' label='Bank' value={selectedPayment.bank} valueClassName='uppercase' />
                  <InfoItem icon='cash' label='Total Pembayaran' value={`Rp ${selectedPayment.amount.toLocaleString()}`} />
                  <InfoItem
                    icon='calendar'
                    label='Tanggal Transfer'
                    value={new Date(
                      selectedPayment.transferDate.split(" ")[0].split("-").reverse().join("-") +
                        "T" +
                  ...
                  <InfoItem
                    icon='calendar'
                    label='Tenggat'
                    value={new Date(selectedPayment.expiryTime).toLocaleDateString("id-ID", {
                      year: "numeric",
                      month: "long",
                      day: "numeric",
                    })}
                  ...
                  />
                  <InfoItem icon='card-outline' label='VA Number' value={selectedPayment.vaNumber} />
                </div>
              </CardBody>
            </Card>
          )}
